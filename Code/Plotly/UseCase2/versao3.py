...
app.layout = html.Div([
    ...
    html.Div([
        dcc.Tabs(id='tabs', value='tab-1', children=[
            ...
            dcc.Tab(label='Compare multiple series', value='tab-2', children=[
                html.Div([
                    dcc.Dropdown(
                        id='series',
                        options=[{'label': value['name'], 'value': key} for key, value in newDataSets.items()],
                    ...
                    style={'width': '200px', 'display': 'inline-block', 'margin-right': '25px'}),
                html.Div([
                    dcc.Graph(id='update_plot2', style={'height': '550px', 'width': '750px', 'position': 'relative'}),
                ], style={'display': 'inline-block'}),
        ...
        html.Div([
            dcc.Graph(id='update_multiple', style={'height': '500px', 'width': '760px', 'overflowY': 'scroll', 'position': 'relative'})
        ], style={'display': 'inline-block', 'margin-top': '100px'}),
    ], style={'display': 'flex'}),
])


def plot(fig, target, chaser, max_value, colorN, flag):
    fig.add_trace(
            go.Scatter(
    ...
    fig.add_trace(
        go.Scatter(
            x=dt_time, y=[warning] * len(dataSets['Neuraspace']['dt_time']), mode='lines', name='Warning Threshold', line=dict(color='#FFB000', dash='dash')
        )
    )

    fig.add_trace(
        go.Scatter(
            x=dt_time, y=[alert] * len(dataSets['Neuraspace']['dt_time']), mode='lines', name='Alert Threshold', line=dict(color='#EA3F6D', dash='dash')
        )
    )
    ...
    y_min = 0
    y_max = max_value + 100
    y_ticks = list(range(int(y_min), int(y_max) + 1, 300))  # Adjust step as needed

    fig.update_yaxes(
        gridcolor='#E5ECF6',
        tickvals=y_ticks, 
        ticktext=[f'{val} m' for val in y_ticks]
    )
...
def update_plot(target, chaser, btn1, btn2, btn3, btn4):
    ctx = dash.callback_context
    if not ctx.triggered:
        button_id = 'btn-1'
    else:
    ...
    if target is None or chaser is None:
        fig.add_trace(
            go.Scatter(
                x=dataSets['Neuraspace']['dt_time'], y=dataSets['Neuraspace']['dt'], mode='lines', name=dataSets['Neuraspace']['name'], line=dict(color=dataSets['Neuraspace']['color'], width=3)
            )
        )
        ...
        if button_id == 'btn-2':
            plot(fig, target, chaser, max_value, colorN, 2)
        elif button_id == 'btn-3':
            plot(fig, target, chaser, max_value, colorN, 3)
        elif button_id == 'btn-4':
            plot(fig, target, chaser, max_value, colorN, 4)
        else:
            plot(fig, target, chaser, max_value, colorN, 1)
    ...
    fig.update_layout(
        ...
        legend=dict(
            x=0,
            y=-0.6,  # Adjusted to ensure it stays just below the graph
            orientation='h',
...
def plot2 (fig, series, flag, max_value, colorN):
    fig.add_trace(
            go.Scatter(
                x=dataSets['Neuraspace']['dt_time'], y=dataSets['Neuraspace']['dt'], mode='lines', name=dataSets['Neuraspace']['name'], line=dict(color=dataSets['Neuraspace']['color'])
            )
    ...
    for s in series:
        color = hex_to_rgba(newDataSets[s]['color'], 0.3)
        fig.add_trace(
            go.Scatter(
                x=newDataSets[s]['dt_time'], y=newDataSets[s]['dt'], mode='lines', name=newDataSets[s]['name'], line=dict(color=color))#, fill='tonexty', fillcolor=color)       
        )
        if flag != 1:
            for index, value in enumerate(newDataSets[s]['upper_bound']):
                if value is None:
                    newDataSets[s]['upper_bound'][index] = newDataSets[s]['dt'][index]
            for index, value in enumerate(newDataSets[s]['lower_bound']):
                if value is None:
                    newDataSets[s]['lower_bound'][index] = newDataSets[s]['dt'][index]

            # Adicionar a banda de erro superior
            fig.add_trace(
                go.Scatter(
                    x=newDataSets[s]['dt_time'],
                    y=newDataSets[s]['upper_bound'],  # Ignorar pontos com 0
                    mode='lines',
                    line=dict(width=0),
                    showlegend=False
                )
...
def update_plot2(selected_series, btn1, btn2, btn3, btn4):
    ctx = dash.callback_context
    if not ctx.triggered:
        button_id = 'btn-1'
    else:
    ...
    if selected_series is None or selected_series == []:
        fig.add_trace(
            go.Scatter(
                x=dataSets['Neuraspace']['dt_time'], y=dataSets['Neuraspace']['dt'], mode='lines', name=dataSets['Neuraspace']['name'], line=dict(color=dataSets['Neuraspace']['color'], width=3)
            )
        ...
        if button_id == 'btn-2':
            plot2(fig, selected_series, 2, max_value, colorN)
        elif button_id == 'btn-3':
            plot2(fig, selected_series, 3, max_value, colorN)
        elif button_id == 'btn-4':
            plot2(fig, selected_series, 4, max_value, colorN)
        else:
            plot2(fig, selected_series, 1, max_value, colorN)
