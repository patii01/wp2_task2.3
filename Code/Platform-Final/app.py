...
def create_hovertext(position, lower_bound, upper_bound, time):
    if lower_bound != upper_bound:
        hover_text = f"Position: {position} m<br>Max: {upper_bound} m<br>Min: {lower_bound} m<br>Time: {time}"
    else:
        hover_text = f"Position: {position} m<br>Time: {time}"
    return hover_text
...
forecast_chaser_graph = html.Div([
     html.Div([
        html.Div([
            html.Label('Switch between single and'),
            html.Label('multiple sources selection'),
        ], className='switch-label'),
        daq.BooleanSwitch(
            id='boolean-switch2',
            on=False,
...
def mainPlot_position(target, chaser, button_id, btn1, btn2, btn3, btn4):
    ...
    if target is None or chaser is None:
        ...
        for dataSet in dataSets_position:
            ...
            fig.add_trace(
                go.Scatter(
                    x=dataSets_position[dataSet]['dt_time'], y=data, mode='markers', name=dataSets_position[dataSet]['name'], marker=dict(color=dataSets_position[dataSet]['color'], size=4))
            )
...
def update_multiple(target, chaser, click_data, button_id, btn1, btn2, btn3, btn4):
    ...
    if n_graphs > 2:
        fig = make_subplots(rows=rows, cols=cols, subplot_titles=[('<b>' + dataSets_position[dataSet]['name'] + '</b>') for dataSet in dataSets_position], horizontal_spacing=0.07)
    elif n_graphs == 1:
    ...
    fig.update_yaxes(gridcolor='#E5ECF6', tickprefix='', ticksuffix=' m', tickfont=dict(size=8))
...
def plot_forecast1(fig, value, selected_points, flag, type, forecast_begin):
    ...
    if time == []:
        fig.add_annotation(
            xref='paper', yref='paper',
            x=0.5, y=0.5,
            text='No data available for the selected source.',
            showarrow=False,
            font=dict(size=20),
            align='center',
            bordercolor='#767689',
        ...
        hover_texts = [
            f"Position: {data['dt'][i]} m<br>Time: {time[i]}"
        ...
        if len(selectPoints) > 0:
            for forecast in selectPoints:
                for m, f in forecast.items():  
                    ...
                    if forecast_data['lower_bound'] == []:
                        forecast_data['lower_bound'] = data['dt']
                    if forecast_data['upper_bound'] == []:
                        forecast_data['upper_bound'] = data['dt']
                    hover_forecast = [
                        create_hovertext(forecast_data['dt'][i], forecast_data['lower_bound'][i], forecast_data['upper_bound'][i], forecast_data['dt_time'][i])
                        for i in range(len(forecast_data['dt']))
                    ]
                    # Forecast
                    fig.add_trace(
                        go.Scatter(
...
def update_output(value, selected_points, button_id, btn1, btn2, btn3):
    ...
    if value is None:
        ...
        for dataSet in dataSets_forecast:
            ...
            fig.add_trace(
                go.Scatter(
                    x=dataSets_forecast[dataSet]['Chaser']['dt_time'], y=data, mode='markers', name=dataSets_forecast[dataSet]['name'], marker=dict(color=dataSets_forecast[dataSet]['color'], size=4))
            )
...
def update_output3(value, selected_points, button_id, btn1, btn2, btn3):
    ...
    if value is None:
        ...
        for dataSet in dataSets_forecast:
            ...
            fig.add_trace(
                go.Scatter(
                    x=dataSets_forecast[dataSet]['Target']['dt_time'], y=data, mode='markers', name=dataSets_forecast[dataSet]['name'], marker=dict(color=dataSets_forecast[dataSet]['color'], size=4))
            )
